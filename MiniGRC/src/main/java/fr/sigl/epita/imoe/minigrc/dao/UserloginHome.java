...
public class UserloginHome {
	...
	public Userlogin merge(Userlogin detachedInstance) {
		...
		try {
			Userlogin result = (Userlogin) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Userlogin findById(int id) {
		...
		try {
			Userlogin instance = (Userlogin) sessionFactory.getCurrentSession().get("Userlogin", id);
			if (instance == null) {
	...
	public List findByExample(Userlogin instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Userlogin").add(Example.create(instance))
					.list();
