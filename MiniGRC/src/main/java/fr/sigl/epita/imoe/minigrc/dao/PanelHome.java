...
/**
 * Home object for domain model class Panel.
 * @see .Panel
 * @author Hibernate Tools
 */
public class PanelHome {

	private static final Log log = LogFactory.getLog(PanelHome.class);
	...
	public void persist(Panel transientInstance) {
		log.debug("persisting Panel instance");
		try {
	...
	public Panel merge(Panel detachedInstance) {
		...
		try {
			Panel result = (Panel) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Panel findById(int id) {
		...
		try {
			Panel instance = (Panel) sessionFactory.getCurrentSession().get("Panel", id);
			if (instance == null) {
	...
	public List findByExample(Panel instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Panel").add(Example.create(instance))
					.list();
